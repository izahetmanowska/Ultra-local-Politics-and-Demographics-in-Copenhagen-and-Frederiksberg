...
Insta CSV Scraper
-----------------
Reads a CSV named "parties_info.csv" with a column "Instagram Tag" (handles without '@').
...
- By default writes to parties_info_scraped.csv to avoid accidental overwrites.

Usage:
  python insta_playwright_csv.py               # reads parties_info.csv, writes parties_info_scraped.csv
...
- If you're starting from a .numbers file, export to CSV first and name it parties_info.csv.
"""
...
def main():
    ap = argparse.ArgumentParser(description="Update parties_info.csv with Instagram follower/post counts.")

    ap.add_argument(
    "--in",
    dest="in_path",
    type=Path,
    default=Path("raw-data/social-media/parties_info.csv"),
    help="Input CSV (default: raw-data/social media/parties_scraper_frb.csv)"
    )
    ...
    dest="out_path",
    type=Path,
    default=Path("raw-data/social-media/parties_info_scraped.csv"),
    help="Output CSV (default: raw-data/social media/parties_scraper_frb_scraped.csv)"
    )
