...
ROOT = '../../../'
PATH = ROOT + 'raw-data/2021-special/'
ELECTION_PATH = PATH + 'ElectionData.csv'
DEMO_PATH = PATH + 'Population.csv'
OUTPUT_PATH = ROOT + 'processed-data/2021-special/'
MUNICIPALITY_IDS = [101, 147] # Copenhagen and Frederiksberg
COMMON_COLS = ['Gruppe', 'ValgstedId', 'KredsNr']
PREFIX = 'KV'

def find_polling_areas():
    df_geo = pd.read_csv(PATH + 'Geography.csv', sep=';')
    cph_areas = set(df_geo[df_geo['KommuneNr'].isin(MUNICIPALITY_IDS)]['Valgsted Id'].astype(str))
    return cph_areas

CPH_AREAS = find_polling_areas()

def inspect_socioeconomy_duplicates(df, year, sample_size=10):
    """
...
def preprocess_socioeconomy2(df, year):
    """
    Extract socioeconomy data: totals for Topledere, Selvstændig, and Arbejdsløse.
    And total across columns.
    Only uses numbered columns (with "01. ", "02. " etc. prefix) to avoid duplicates.
    """
    pattern = f'{PREFIX}{year} - Socio-økonomisk status og brancher fordelt på afstemningsområder_'
    df = df[df['Gruppe'].isin(CPH_AREAS)]
    ...
    for category in all_categories:
        ...
        if not sub_cols:
            print(f"    Warning: No numbered columns found for {category}")
            continue
...
def inspect_citizenship_duplicates_simple(df, year, sample_size=10):
    """
...
def preprocess_citizenship_sex_age(df, year):
    """
...
def inspect_education_duplicates(df, year, sample_size=10):
    """
...
def preprocess_education(df, year):
    """
...
def inspect_immigrants_duplicates(df, year, sample_size=10):
    """
...
def preprocess_immigrants(df, year):
    """
...
def inspect_housingtenure_duplicates(df, year, sample_size=10):
    """
...
def preprocess_housingtenure(df, year):
    """
...
def inspect_housingtype_duplicates(df, year, sample_size=10):
    """
...
def preprocess_housingtype(df, year):
    """
...
def inspect_constructionyear_duplicates(df, year, sample_size=10):
    """
...
def preprocess_builtyear(df, year):
    """
    Extract housing type data per year.
    """    
    pattern = f'{PREFIX}{year} - Opførelsesår_'
...
def preprocess_simple(df, year):
    ...
    """
    result_df = df[df['Gruppe'].isin(CPH_AREAS)][COMMON_COLS].copy()
    ...
    for name, config in SIMPLE_CSVS.items():
        ...
        for new_name, col_suffix in config['columns'].items():
            ...
            out_col = f'{prefix}_{new_name}'

            if vals is not None:
                result_df[out_col] = finalise_column(vals)
            else:
                print(f"      Warning: Column '{full_col_name}' not found")
                result_df[out_col] = np.nan
